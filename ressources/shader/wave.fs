...
void main() {
    ...
    p.x += cos((fragTexCoord.y - boxTop) * main_freq.x * pixelWidth + (time * main_vel.x)) * main_amp.x * pixelWidth;
    p.y += sin((fragTexCoord.x - boxLeft) * main_freq.y * pixelHeight + (time * main_vel.y)) * main_amp.y * pixelHeight;
