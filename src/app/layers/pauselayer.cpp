...
void PauseLayer::on_render()
{
	...
	*/
	...
	if ( m_active ) {
		if ( GuiButton(
					 Rectangle{screen_width / 2 - (40 * scale), screen_height / 2 - (12 * scale) - 40 * scale, 80 * scale, 24 * scale}, "Resume"
			 ) ) {
			gApp()->call_later([]() {
		...
		if ( GuiButton(Rectangle{screen_width / 2 - (40 * scale), screen_height / 2 - (12 * scale), 80 * scale, 24 * scale}, "Main Menu") ) {
			gApp()->call_later([]() {
				gApp()->pop_layer<PauseLayer>();
		...
		if ( GuiButton(
					 Rectangle{screen_width / 2 - (40 * scale), screen_height / 2 - (12 * scale) + 40 * scale, 80 * scale, 24 * scale}, "Scoreboard"
			 ) ) {
			gApp()->call_later([]() {
				...
				if ( score_layer ) {
					score_layer->load_highscore(score_layer->score_filename());
					score_layer->layout = ScoreLayer::ESC_MENU;
				}
			});
			m_active = false;
		}
